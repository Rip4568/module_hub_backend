import { Controller, Get, UseGuards, Request, Query, Res, BadRequestException, UnauthorizedException } from '@nestjs/common';
...
export class DriverPortalController {
    ...
    async getMe(@Request() req: AuthenticatedRequest) {
        ...
        if (!tenantId) {
            throw new BadRequestException('Tenant ID header (x-tenant-id) is required for this operation');
        }
        if (!req.user?.userId) throw new UnauthorizedException('User ID missing');
     ...
     */
    @Get('export')
    async exportData(@Request() req: AuthenticatedRequest, @Query('format') format: 'xlsx' | 'csv' = 'xlsx', @Res() res: Response) {
        const tenantIdHeader = req.headers['x-tenant-id'];
        const tenantId = Array.isArray(tenantIdHeader) ? tenantIdHeader[0] : tenantIdHeader;
